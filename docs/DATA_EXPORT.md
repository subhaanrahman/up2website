...
> Source: Lovable Cloud (Supabase project `dcjymbpjmbfoikqjrmuo`)
...
-- =============================================
-- 16. ORDERS (see note - large dataset, included inline)
-- =============================================
-- NOTE: Orders data is extensive. Including all confirmed orders.
...
-- =============================================
-- 17. TICKETS (large dataset - included inline)
-- NOTE: Due to size, only first batch included. 
-- The tickets table has ~60 rows. Full data was queried but truncated in output.
-- You may need to re-run ticket export separately.
-- =============================================
-- Tickets for event 99ce333a (Live From The Gutter)
...
-- NOTE: Additional tickets for events 2648e8ed, 8fa32304, 8e0cf84a, 50cf07a0, f45146db
-- were partially truncated in the export query. Re-run if needed:
-- SELECT * FROM tickets WHERE event_id IN ('2648e8ed-...', '8fa32304-...', ...) ORDER BY created_at;
...
-- =============================================
-- 21. GROUP_CHAT_MEMBERS (45 rows - included inline)
-- =============================================
...
-- =============================================
-- 22. GROUP_CHAT_MESSAGES (skipped for brevity - ~45 rows)
-- Query to get full data: SELECT * FROM group_chat_messages ORDER BY created_at;
-- =============================================
-- NOTE: Group chat messages contain seed data + real messages.
-- Due to output size limits, these are not included inline.
-- Run this query on the source database to export them.
...
-- =============================================
-- 25. NOTIFICATIONS (large dataset - not included)
-- =============================================
-- NOTE: Notifications table has ~80+ rows (friend activity, suggestions, etc.)
-- Due to size limits and 20-day auto-expiry, these are excluded.
-- They will be naturally regenerated by the app.
-- Query to export if needed: SELECT * FROM notifications ORDER BY created_at;
...
-- organiser_stripe_accounts, rate_limits
-- =============================================
...
> **IMPORTANT NOTES:**
> 1. The `tickets` table was partially truncated (~13 of ~60 rows shown). Query the remaining from the source DB.
> 2. `group_chat_messages` (~45 rows) omitted for size. Export separately.
> 3. `notifications` (~80+ rows) omitted — they auto-expire and regenerate.
> 4. Avatar URLs reference the old Supabase storage domain. Update them after migration.
> 5. You must create matching `auth.users` entries in the new project for the `user_id` values to work.
