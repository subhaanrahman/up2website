...
This plan does **not** treat compute scaling as the primary fix. Right-sizing is a safety valve only after evidence review.
...
- `src/hooks/useUnreadMessages.ts`
  ...
  - performs per-thread unread count loops against `group_chat_messages` and `dm_messages`
...
- `src/hooks/useNotificationsQuery.ts`
  - loads `select("*")`
  - invalidates the full list on realtime changes
- `src/hooks/useFeedQuery.ts`
  - subscribes broadly to `posts` and `post_reposts`
  - invalidates the whole feed query family
...
- Replace unread polling plus per-thread count loops with a server-side unread-count path.
- Paginate DM, group chat, and event board message history instead of reloading full history.
- Narrow realtime subscriptions and query invalidation so only the affected thread, user scope, or feed slice refreshes.
- Remove remaining `select("*")` on hot notification and messaging paths where explicit columns or RPCs are enough.
- Review `dashboard-analytics` and move repeated fan-out reads toward pre-aggregated/RPC-backed paths.
...
- If Disk IO pressure becomes operationally unsafe before the root cause work lands, a temporary compute upgrade is allowed only as a safety valve and must not be recorded as the primary remediation.
