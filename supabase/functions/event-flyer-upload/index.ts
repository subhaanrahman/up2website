...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const allowed = await checkRateLimit('event-flyer-upload', user.id, getClientIp(req));
    ...
    ensureStoragePathPrefix(parsed.data.path, `${user.id}/event-flyers`);
    const publicUrl = await getPublicStorageUrl(serviceClient, 'event-flyers', parsed.data.path);
    return successResponse({ success: true, url: publicUrl }, requestId);
  } catch (err) {
    edgeLog('error', 'event-flyer-upload error', { requestId, error: String(err) });
